...
**Why this matters:** The system processes ONE reaction per execution. For batch processing, you'd run this script multiple times (e.g., in parallel using a job scheduler).
...
**Why:** Coefficients like `2`, `n`, `m+1` confuse the atom mapping algorithms. We only care about which molecules participate, not their quantities.
...
**Purpose:** Allows multiple instances of this script to run simultaneously without corrupting output files
...
### File Location
The script searches for files in:
...
### Long-Term Dataset Results

**After processing 1000s of reactions, you'll have:**

1. **Main insights:**
   - Which atoms are conserved across reactions
   - Common transformation patterns (e.g., phosphate transfer)
   - Reaction classes based on atom fate

2. **Quality metrics:**
   - Success rate of atom mapping
   - Frequency of RXNMapper vs. localmapper usage
   - Common failure modes

3. **Cofactor analysis:**
   - Impact of cofactor removal on tracking success
   - Identification of true cofactors (static vs. dynamic)

4. **Visualization library:**
   - Catalog of atom-mapped reactions
...
### 1. Start Small
- Test with `--row_index 0` first
- Verify output format is as expected
- Check error handling with known problematic reactions

### 2. Use Appropriate Cofactor Filtering
- **Metabolic reactions:** Use `--static_cofactors`
- **Synthetic reactions:** Use `--dynamic_cofactors`
- **Unknown domain:** Try both and compare results

### 3. Choose Selection Method Wisely
- **Default:** Good for most cases
- **--highest_CCC:** For stereochemistry studies
- **--lowest_CCC:** For small molecule tracking

### 4. Validate Results
- Use `--visualize` on random samples
- Manually check a few tracking results
- Compare with literature or databases

### 5. Parallel Processing
- This script is designed for single-reaction processing
- Safe to run multiple instances simultaneously (FileLock protection)
- Recommended: 4-8 parallel jobs depending on CPU cores

### 6. Monitor Error File
- Regularly check `Error Reactions.csv`
- Common errors indicate systematic issues (e.g., missing SMILES)
- Fix data quality issues before full batch processing

---

## Performance Considerations

**Processing time per reaction:**
- Simple reaction (< 20 atoms): 1-3 seconds
- Medium reaction (20-40 atoms): 3-8 seconds
- Large reaction (> 40 atoms): 8-20 seconds (if using localmapper)

**Bottlenecks:**
- RXNMapper API calls (if using online version)
- First run canonical index building (one-time cost)
- Visualization PNG generation (if enabled)

**Optimization tips:**
- Pre-build canonical index before batch processing
- Skip visualization for bulk processing
- Use SSD storage for faster CSV I/O
- Consider disabling FileLock if running single-threaded
...
- **RXNMapper:** Schwaller, P., et al. (2021). "Mapping the space of chemical reactions using attention-based neural networks"
- **localmapper:** Graph-based atom mapping algorithm

**Use case:** This script is designed for biochemical reaction databases (CBRdb, KEGG, etc.) but can be adapted for any reaction dataset.

---

## Conclusion

This atom tracking system provides a **comprehensive, automated solution** for understanding atom fate in chemical reactions. By combining:

- **State-of-the-art AI mapping** (RXNMapper)
- **Robust fallback mechanisms** (localmapper)
- **Flexible cofactor handling**
- **Multiple substrate selection strategies**
- **Detailed error reporting**

...it enables large-scale analysis of reaction mechanisms, metabolic pathways, and chemical transformations.

The output clearly shows **which compound a selected atom ends up in**, enabling downstream analyses such as:
- Metabolic flux tracking
- Isotope labeling studies
- Mechanism-based reaction classification
- Drug metabolism prediction
- Biosynthetic pathway design

**Key takeaway:** Given a reaction like `A + B ⇄ C + D`, this script answers: *"If I track a specific atom in molecule A, does it end up in C or D?"*
